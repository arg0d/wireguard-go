...
again:
	...
	var fns []ReceiveFunc
	if ipv4 != nil {
		fns = append(fns, makeReceiveIPv4(ipv4))
		bind.ipv4 = ipv4
	}
	if ipv6 != nil {
		fns = append(fns, makeReceiveIPv6(ipv6))
		bind.ipv6 = ipv6
	}
	if len(fns) == 0 {
...
func makeReceiveIPv4(conn *net.UDPConn) ReceiveFunc {
...
func makeReceiveIPv6(conn *net.UDPConn) ReceiveFunc {
	return func(buff []byte) (int, Endpoint, error) {
		n, endpoint, err := conn.ReadFromUDP(buff)
		return n, (*StdNetEndpoint)(endpoint), err
	}
